    ...
    fs.readFile("./src/model/pokedex.json",(err,data) =>{
        ...
        if (err){
            ...
            // sinon envoie contenu du fichier avc status 200
                ...
                response.status(200).json(JSON.parse(data).spectre)
            }
    })
}
exports.getTypeByName=(request, response) =>{
    ...
    fs.readFile('./src/model/pokedex.json',(err,data)=>{
        if(err){
            ...
            const data_name=manip_data.spectre.find(
  ...
  fs.readFile("./src/model/pokedex.json",(err,data)=>{
        ...
        if (err){
            ...
            response.status(500).json({
                ...
                const data_id= manip_data.spectre.find(
...
exports.updateData= (request, response) => {
    ...
    fs.readFile("./src/model/pokedex.json", (err,data)=>{
        ...
        const data_id = existing_data.spectre.find(
...
exports.DeleteById=(request, response)=>{
    ...
    if (err){  //si erreur renvoie status 500 + message d'erreur
        ...
        if(!data_id){
            ...
            existing_data.spectre=existing_data.spectre.filter(
                (obj)=> obj.id != parseInt(request.params.id));
...
exports.CreateData=(request,response)=> {
    ...
    fs.readFile("./src/model/pokedex.json",(err,data)=>{
        if(err){
            ...
            //transforme la data en JSON manipulable
            const existing_data= JSON.parse(data);
            //enregistre le tableau spectre seulement
            const dataID = existing_data.spectre
            ...
            existing_data.spectre.push({
            "id":LastId,
            "name": request.body.name,
            "numero": request.body.numero
            })
            //rajouter cette donnée a partir de ce qu'on a ecrit dans le body
            existing_data.spectre.push(request.body);
